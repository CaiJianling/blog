...
interface WhalePoint {
    x: number;
    y: number;
    /** 每个点的尺寸随机因子，让粒子大小不一、更自然。 */
    size: number;
}

/** 首字母点阵的采样分辨率：点阵横跨剪影的点数，越大粒子越小、数量越多。 */
const SAMPLE_SIZE = 110;
...
interface Palette {
    ...
    gridLine: string;
    gridDot: string;
    letterIdleAlpha: number;
    letterIdleAlphaCompact: number;
    letterRevealAlpha: number;
    letterDotIdleRGB: [number, number, number];
    letterDotRevealRGB: [number, number, number];
...
const DARK_PALETTE: Palette = {
    ...
    letterIdleAlpha: 0.07,
    letterIdleAlphaCompact: 0.05,
    letterRevealAlpha: 0.5,
    letterDotIdleRGB: [205, 216, 230],
    letterDotRevealRGB: [163, 205, 255],
    whaleGlowStops: [
        'rgba(230,237,246,',
        'rgba(188,204,224,',
        'rgba(97,124,160,0)',
...
const LIGHT_PALETTE: Palette = {
    ...
    ribbonColors: [
        'rgba(100, 130, 175, 0.12)',
        'rgba(70, 105, 150, 0.1)',
        'rgba(160, 180, 205, 0.1)',
    ],
    ribbonWidths: [0, 0, 0],
    ...
    letterIdleAlpha: 0.09,
    letterIdleAlphaCompact: 0.07,
    letterRevealAlpha: 0.6,
    letterDotIdleRGB: [71, 85, 105],
    letterDotRevealRGB: [59, 130, 246],
    whaleGlowStops: [
        'rgba(147, 197, 253,',
        'rgba(191, 219, 254,',
        'rgba(226, 232, 240, 0)',
...
/**
 * 画布尚未绘制时承载画布的 section 所需回退底色。
 * 取流体渐变的顶部色（文案所在区域），与画布首帧渲染一致，避免亮色主题下深色底闪现。
 */
export function heroFallbackBackground(dark: boolean): string {
    const stops = dark ? DARK_PALETTE.baseStops : LIGHT_PALETTE.baseStops;
...
export default function HeroCanvas({ name = '' }: HeroCanvasProps) {
    ...
    useEffect(() => {
        ...
        const drawFluid = (time: number) => {
            ...
            pointerEase.x +=
                ((pointer.active ? pointer.x : width * 0.62) - pointerEase.x) *
                0.025;
            pointerEase.y +=
                ((pointer.active ? pointer.y : height * 0.22) - pointerEase.y) *
                0.025;
            ...
            const ink = fluidCtx.createRadialGradient(
                inkX,
                inkY,
                0,
                inkX,
            ...
            const warm = fluidCtx.createRadialGradient(
                warmX,
                warmY,
                0,
                warmX,
            ...
            cursorGlow.addColorStop(
                0,
                pointer.active ? P.cursorGlowActive : P.cursorGlowIdle,
            );
            ...
            const grainCount = Math.min(
                420,
                Math.round((width * height) / 2400),
            );

            for (let grain = 0; grain < grainCount; grain += 1) {
                fluidCtx.fillRect(
                    hash(grain + 4) * width,
                    hash(grain + 307) * height,
                    1,
                    1,
                );
            }
        ...
        const sampleLetter = () => {
            ...
            // 名称过长时缩小字号以适配采样宽度（字号随采样分辨率等比放大）
            let fontSize = Math.round(sampleSize * 0.66);
            ...
            for (let y = 0; y < sampleSize; y += 1) {
                for (let x = 0; x < sampleSize; x += 1) {
                    if (luminance[y * sampleSize + x] <= 0.2) {
                        continue;
                    }
                    ...
                    for (let oy = -2; oy <= 2; oy += 1) {
                        for (let ox = -2; ox <= 2; ox += 1) {
                            ...
                            if (
                                ...
                                nx < sampleSize &&
                                ny < sampleSize &&
                                luminance[ny * sampleSize + nx] > 0.2
                            ) {
                                neighbors += 1;
                            }
                        }
                    ...
                    // 预计算不规则尺寸因子：整体偏小、幂函数进一步压低，偶有大点
                    const sizeSeed = y * sampleSize + x + 1;
                    const size = 0.4 + Math.pow(hash(sizeSeed + 53), 1.7) * 1.5;

                    whalePoints.push({ x, y, size });
                }
            }
        };
        ...
        const drawWhale = (time: number) => {
            ...
            if (!whalePoints.length) {
                return;
            }

            const idleAlpha = compact.matches
                ? P.letterIdleAlphaCompact
                : P.letterIdleAlpha;
            // 以屏幕宽度为基准自适应
            const silhouetteSize = compact.matches
                ? width * 0.96
                : width * 0.92;
            const pointStep = silhouetteSize / SAMPLE_SIZE;
            // 点阵居中（位于文案后方，低透明度作背景装饰）
            const centerX = width * 0.5;
            const centerY =
                height * (compact.matches ? 0.3 : 0.44) +
                Math.sin(time * 0.4) * (compact.matches ? 3 : 7);
            const groupScale = 0.956;
            const rotation = 0.04 * Math.sin(time * 0.25);

            const pointerTarget = whaleHovered && pointer.active ? 1 : 0;
            const pointerEaseRate =
                pointerTarget > pointerStrength ? 0.3 : 0.14;

            pointerStrength +=
                (pointerTarget - pointerStrength) * pointerEaseRate;
            ...
            whaleCtx.scale(
                groupScale,
                groupScale * (1 + Math.sin(time * 0.08) * 0.025),
            );

            if (pointerStrength > 0.01 && !reduceMotion.matches) {
                const glow = whaleCtx.createRadialGradient(
                    mouseX,
                    mouseY,
                    0,
                    mouseX,
            ...
            whalePoints.forEach((point) => {
                const targetX = (point.x - SAMPLE_SIZE / 2) * pointStep;
                const targetY = (point.y - SAMPLE_SIZE / 2) * pointStep;
                let x = targetX;
                let y = targetY;
                ...
                if (pointerStrength > 0.01 && !reduceMotion.matches) {
                    ...
                    if (mouseDistance < mouseRadius && mouseDistance > 0.1) {
                        ...
                        mouseInfluence =
                            ...
                            (3 - 2 * mouseFalloff);
                        wave = Math.sin(
                            (mouseDistance / pointStep) * 1.35 - time * 10.5,
                        );

                        const direction = Math.atan2(mouseDy, mouseDx);
                        const seed = point.y * SAMPLE_SIZE + point.x + 1;
                        const randomAngleA = hash(seed + 23) * Math.PI * 2;
                        const randomAngleB = hash(seed + 211) * Math.PI * 2;
                        const randomPulseA = Math.sin(
                            time * (4.8 + hash(seed + 67) * 3.2) +
                                hash(seed + 101) * Math.PI * 2,
                        );
                        const randomPulseB = Math.cos(
                            time * (5.6 + hash(seed + 307) * 2.6) +
                                hash(seed + 401) * Math.PI * 2,
                        );
                        const randomForce =
                            mouseInfluence * activity * pointStep * 3.05;
                        const rippleForce =
                            mouseInfluence * activity * pointStep * wave * 0.65;

                        x +=
                            Math.cos(randomAngleA) * randomPulseA * randomForce;
                        y +=
                            Math.sin(randomAngleA) * randomPulseA * randomForce;
                        x +=
                            Math.cos(randomAngleB) *
                            randomPulseB *
                            randomForce *
                            0.42;
                        y +=
                            Math.sin(randomAngleB) *
                            randomPulseB *
                            randomForce *
                            0.42;
                ...
                // 揭示度：全局缓动（鼠标进入/离开渐入渐出）×（整体显色 + 鼠标热点）。
                // 未 hover 时 pointerStrength≈0 → 揭示度≈0，点保持淡色；鼠标侵入后
                // 整块点阵渐显主题色，鼠标附近进一步加亮（热点），离开再渐隐。
                ...
                const pointAlpha = Math.min(
                    P.letterRevealAlpha,
                    idleAlpha + (P.letterRevealAlpha - idleAlpha) * reveal,
                );
                const pointSize = Math.max(1.1, pointStep * 0.28 * point.size);

                whaleCtx.fillStyle = `rgba(${r}, ${g}, ${b}, ${pointAlpha.toFixed(3)})`;
        ...
        if (window.ResizeObserver) {
            ...
            window.addEventListener('resize', onCompactChange, {
                passive: true,
            });
        }
    ...
    return (
        ...
        >
            <canvas
                ref={fluidRef}
                className="absolute inset-0 block h-full w-full opacity-[0.98]"
            />
            <canvas
                ref={gridRef}
                className="absolute inset-0 block h-full w-full [mask-image:linear-gradient(rgba(0,0,0,0.99)_0%,rgba(0,0,0,0.91)_9%,transparent_100%)] opacity-90"
            />
            <canvas
                ref={whaleRef}
